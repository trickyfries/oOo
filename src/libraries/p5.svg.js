(function () {
  ...
  class ImageUtils {
    ...
    getImageData(svgNode, width, height, sx, sy, sw, sh, options) {
      ...
      if (!options.async) {
        throw new Error(
          "svgcanvas: options.async must be set to true for getImageData"
        );
      }
      const svgDataURL = this.toDataURL(
        svgNode,
        width,
        height,
        "image/svg+xml"
  ...
  var Context = (function () {
     ...
     * @private
     */
    Context.prototype.__createElement = function (
      elementName,
      properties,
      resetFill
    ) {
        ...
        if (style.apply) {
          ...
          if (value instanceof CanvasPattern) {
            ...
            if (value.__ctx) {
              ...
              for (
                nodeIndex = 0;
                nodeIndex < value.__ctx.__defs.childNodes.length;
                nodeIndex++
              ) {
    ...
    Context.prototype.restore = function () {
      ...
      if (
        this.__transformMatrixStack &&
        this.__transformMatrixStack.length > 0
      ) {
   ...
   */
  SVGCanvasElement.prototype.toDataURL = function (
    type,
    encoderOptions,
    options
  ) {
    return utils.toDataURL(
      this.svg,
      this.width,
      this.height,
      type,
  ...
  function RendererSVG(p5) {
     ...
     * @param {Number} height
     */
    RendererSVG.prototype.image = function (
      img,
      sx,
      sy,
      sWidth,
      ...
      elt =
        ...
        (img.elt &&
          img.elt.nodeName &&
          img.elt.nodeName.toLowerCase() === "svg" &&
          img.elt); // SVGElement
    ...
    RendererSVG.prototype.loadPixels = async function () {
      ...
      const imageData = await this.drawingContext.getImageData(0, 0, w, h, {
        async: true,
      });
  ...
  function IO(p5) {
    ...
    p5.prototype._makeSVGFrame = function (options) {
      ...
      if (!mine) {
        throw new Error(
          "Fail to getFrame, invalid extension: " +
            ext +
            ", please use png | jpeg | jpg | svg."
        );
      }
    ...
    p5.prototype.save = function () {
      ...
      var useSVG =
        ...
        supportedExtensions.indexOf(ext) > -1;
  ...
  function P5SVGFilters(p5) {
    ...
    SVGFilters.apply = function (svgElement, func, arg, defs) {
      ...
      filters = filters.map(function (filter, index) {
        var inGraphics =
          index === 0 ? "SourceGraphic" : "result-" + (index - 1);
        var resultGraphics = "result-" + index;
        return SVGFilters[filter[0]].call(
          null,
          inGraphics,
          resultGraphics,
          filter[1]
